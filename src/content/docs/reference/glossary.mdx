...
| **Builtin Clip** | Clip running inside the `pinixd` process (Go), registered via `builtin.Registry` | Not an SDK Clip, no IPC |
...
| **Agent Runtime** | LLM execution engine (Go). Local: builtin Clip `agent` in pinixd. Cloud: pinix-api `/ai/*` | Not a Clip (conceptually), though registered as one |
| **Scheduler** | Cron-based task scheduler, builtin Clip in pinixd | Not a separate service |
