...
<Aside type="caution">
  **Reconnection is mandatory.** If the ProviderStream disconnects, the Provider must automatically reconnect with exponential backoff. `pinixd` never exits due to network errors — this is a design invariant.
</Aside>
