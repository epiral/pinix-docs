...
2. **登录**
   ...
   打开浏览器进行授权。登录后，daemon 自动连接 Cloud Hub——你的本地 Clip 从云端也能访问，你也能用到别人共享的 Clip。

   <Aside type="tip">
     也可以用 token 登录：`pinix login --token pnx_...`（从 [pinixai.com/settings](https://pinixai.com/settings) 获取）。
   </Aside>
...
只安装 `pinix` 命令行工具，不启动 daemon，不装运行时。通过远程 Hub 调用别人的 Clip。
...
安装后，通过 `--server` 指定远程 Hub 地址来调用 Clip：
...
<Aside type="tip">
  ...
  - 在 CI/CD 中调用 Clip
  - 在轻量设备上远程使用 Clip
  - 在 Windows 上远程调用 macOS/Linux 上跑的 Clip
</Aside>
